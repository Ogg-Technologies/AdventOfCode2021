...
fun main() {
    val lines = File("src/prob11/prob11Input.txt").readLines()
    //part1(lines)
    part2(lines)
}
...
fun Grid<Squid>.next() {
    this.forEach { squid -> squid.energy++ }

    this.forEachIndexed { pos, squid ->
        calculateSpreadPositions(
            startPosition = pos,
            neighborDeltas = neighborDeltas,
            spreadAllowed = { from, to -> !from.value.hasFlashed && from.value.energy > 9 },
            onSpread = { from, to ->
                to.value.energy++
                from.value.hasFlashed = true
            }
        )
    }

    if (this.iterator().asSequence().all { it.hasFlashed }) {
        synchronized = true
    }
...
class Grid<T>(val size: Vector, private val grid: Array<Array<T>>) {
    ...
    fun calculateSpreadPositions(
        ...
        onSpread: (from: GridElement<T>, to: GridElement<T>) -> Unit = { _, _ -> }
    ): Set<Vector> {
        fun neighborsTo(pos: Vector) = neighborDeltas.map { pos + it }
        fun spread(pos: Vector) {
            neighborsTo(pos)
                .filter { neighbor ->
                    neighbor isWithin this && spreadAllowed(
                        GridElement(pos, this[pos]),
                        GridElement(neighbor, this[neighbor])
                    )
                }
                .forEach {
                    onSpread(GridElement(pos, this[pos]), GridElement(it, this[it]))
                    spread(it)
                }
        }
        spread(startPosition)
        return emptySet()
    }
    ...
    fun toString(elementTransformer: (T) -> String): String {
        return grid.fold("") { acc, row ->
            acc + row.fold("") { rowAcc, value ->
                rowAcc + elementTransformer(value)
            } + "\n"
        }
    }
...
infix fun <T> Vector.isWithin(grid: Grid<T>) =
    intY in 0 until grid.size.intY && intX in 0 until grid.size.intX

infix fun <T> Vector.isNotWithin(grid: Grid<T>) = !(this isWithin grid)
